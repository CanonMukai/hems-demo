...
def solve_DA():
    ...
    def make2Table(schedule,start):
        ...
        table1 = makeTable_(start,[demand,sun,cost],label,0)
        label = ["太陽光使用(w)","太陽光充電(w)","太陽光売電(w)",\
                "蓄電池使用(w)","商用電源使用(w)","商用電源充電(w)",\
                "蓄電池残量(w)"]
        table2 = makeTable_(start,unitDouble(schedule),label,1)  
    ...
    def output(opt_result,start):
        ...
        #表表示
        table1, table2 = make2Table(schedule,start)
        #棒グラフ表示
        # bar = makeBar(schedule,start,D,Sun)
        return table1, table2#, bar
    ...
    for i in range(len(answer)):
        ...
        #結果出力
        table1, table2 = output(opt_result,startTime)
