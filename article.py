...
''',
    },
    {
        'type': 'text',
        'body': r'''
$C_\textrm{env}$は商用電源を使用する際の$\textrm{CO}_{2}$排出量に基づく係数である。その指標として$\textrm{CO}_{2}$排出係数を用いる。この項ではスケジュール通りに運用した場合に排出される$\textrm{CO}_{2}$排出量を計上する。変数$a$は$H_\mathrm{envc}$に対し$H_\mathrm{expc}$を優先する度合いを示し、0以上1以下の値をとる。$a$を調節することでスケジューリングにおける経費面と環境面の優先度合を決定する。
''',
    },
    {
        'type': 'text',
...
''',
    },
    {
        'type': 'text',
        'body': r'''
$D$は需要量、$\textrm{Use}$は集合$\{$太陽光使用$1$ ,..., 太陽光使用$n_{1}$, 蓄電池使用$1$ ,..., 蓄電池使用$n_{4}$, 商用電源使用$1$ ,..., 商用電源使用$n_{5}\}$である。</p><br>
...
''',
    },
    {
        'type': 'text',
        'body': r'''
$S$は太陽光発電量、$\textrm{Sun}$は集合$\{$太陽光使用$1$ ,..., 太陽光使用$n_{1}$, 太陽光充電$1$ ,..., 太陽光充電$n_{2}$, 太陽光売電$1$ ,..., 太陽光売電$n_{3}\}$である。</p><br>
